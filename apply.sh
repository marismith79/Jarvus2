...
original_author="marismith79"
original_name="jarvus2"
original_urlname="Jarvus2"
original_description="Awesome jarvus2 created by marismith79" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/jarvus2 "${name}"
...
then
    ...
    echo "then run 'jarvus2' or 'python -m jarvus2'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'jarvus2' or 'python -m jarvus2' or 'poetry run jarvus2'"
fi
