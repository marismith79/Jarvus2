...
def grant_tool_access(user_id, tool_name, features, permission_type='read', duration_days=None):
    """
    ...
    if permission_type not in PERMISSION_TYPES:
        raise ValueError(f"Invalid permission type. Must be one of: {list(PERMISSION_TYPES.keys())}")
    
    if tool_name not in TOOL_FEATURES:
        raise ValueError(f"Invalid tool name. Must be one of: {list(TOOL_FEATURES.keys())}")
    ...
    for feature in features:
        if feature not in TOOL_FEATURES[tool_name]:
            raise ValueError(f"Invalid feature '{feature}' for tool '{tool_name}'")
