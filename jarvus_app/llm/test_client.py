    ...
    try:
        ...
        messages = [
            ...
            client.format_message(
                "user", "Hello! Can you tell me what 2+2 is?"
            ),
        ]
...
def test_mcp_integration():
    ...
    try:
        ...
        messages = [
            client.format_message(
                "system",
                "You are a helpful assistant that can execute commands on the MCP server.",
            ),
            client.format_message(
                "user", "Can you list the contents of the current directory?"
            ),
        ]

        # Get response from OpenAI with tools enabled
        response = client.create_chat_completion(messages, tools=tools)
